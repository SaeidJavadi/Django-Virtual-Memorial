    ...
    if request.method == 'POST':
        ...
        try:
            if request.POST['quran']:
                ...
                # sorted(jozs.values(), reverse=True)

                # print('max',sorted(jozs.values(), reverse=True)[:1])
                # print('min',sorted(jozs.values())[:1])
                max_key = max(jozs, key=jozs.get)
                print(max_key)
                # print('min',jozs[min])
                # jMax = 0
                # jMin = 0
